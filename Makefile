...
BONUSCPPFLAGS := -Ibonus/includes -Ilibft/includes -I$(MLXDIR)
...
$(BONUSNAME): $(BONUSOBJS) $(LIBFT) $(MLX)
	$(CC) $(CLAGS) $(BONUSCPPFLAGS) $(BONUSOBJS) $(LDLIBS) $(LDFLAGS) -o $(BONUSNAME) $(COMPFLAGS)

$(BONUSOBJDIR)%.o: $(BONUSSRCDIR)%.c | $(BONUSBUILDDIR) $(BONUSOBJDIR) $(BONUSDEPDIR)
	$(CC) $(CFLAGS) $(BONUSCPPFLAGS) $(DEPSFLAGS) $(BONUSDEPDIR)$*.d -c $< -o $@ $(COMPFLAGS)
